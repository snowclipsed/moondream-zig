            ...
            imageSession: while (true) {
                ...
                chatLoop: while (true) {
                    ...
                    { // Inference scope
                        ...
                        var current_embeds = try input_embeds.copy();
                        defer current_embeds.deinit();

                        for (0..max_tokens) |_| {
                            if (pos >= 2048) break;

                            // Handle text decoder result
                            var result = try text_model.text_decoder(current_embeds, &kv_cache);
                            ...
                            defer logits.deinit();
                            const next_token = try ops.sample(f16, &logits, sampling_config, random, allocator);
                            ...
                            next_token_tensor.data[0] = @intCast(next_token);

                            // Get new embeddings
                            var new_embeds = try text_model.text_encoder(next_token_tensor);
                            defer {
                                current_embeds.deinit();
                                if (pos < max_tokens - 1) {
                                    // Don't set current_embeds to undefined if this is the last iteration
                            ...
                            try token_list.append(@intCast(next_token));
                            const decoded = try tokenizer.decode(token_list);
                    ...
                    } // End of inference scope
                } // End of chat loop
            } // End of image session
        }
    }
}
